...
- 物料标准名称字典 v0.5 已覆盖正式发布表全部 `147` 个物料族和 `1,979` 个 SKU：当前 `940` 个 SKU 已通过生成、独立复核和程序校验形成冻结映射，其余 `1,039` 个 SKU 均有逐条问题记录，未解释 SKU 为 `0`；治理结果只进入 PostgreSQL 与 Git 快照，尚未改写正式物料表或 ERPNext。
