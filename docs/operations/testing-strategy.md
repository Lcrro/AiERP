...
7. 真实 LLM 评测必须提供终态证据：读操作实际执行预期工具或取得允许的实时 Resolver 数据，写操作生成预期待确认 ToolCall；能力发现和自然语言回答本身不算成功。
8. 修复模型稳定性问题时先跑单元测试和一个对应真实场景；完整 20 次基准只在阶段边界或发布候选运行，避免重复产生 API 成本。
