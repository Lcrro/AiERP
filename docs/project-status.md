...
- 物料标准名称字典 v0.5 已覆盖正式发布表全部 `147` 个物料族和 `1,979` 个 SKU：当前 `1,586` 个 SKU 已通过生成、独立复核和程序校验形成冻结映射，其余 `393` 个 SKU 均有逐条问题记录，未解释 SKU 为 `0`。新增问题修复模式将未决项按族和名称压缩后并发治理，三轮分别新增 `326`、`217`、`43` 个冻结映射；第三轮增益低于 `10%` 后停止自动循环。剩余问题集中于 `9` 个真实边界争议族。治理结果只进入 PostgreSQL 与 Git 快照，尚未改写正式物料表或 ERPNext。
