...
- Python 全量回归 `490 passed`；Tool Schema 与 Adapter handler 为 `161 / 161` 且无缺失或多余项，Plugin 测试 `5 passed`，TypeScript 构建检查通过。
- 标准物料建档 `op.material.create_item` 已接入同一说明书 Runtime：分类、查重、编码、物料组和单位均由确定性服务生成；7 个字段来源与 4 条跨字段规则已入 PostgreSQL，模型只能提供客观描述与已确认属性。
- 真实 Capability API 建档预览已停在不可修改确认卡并验证 ERPNext 依赖；本轮没有写入 Item，临时 pending 已清理。
