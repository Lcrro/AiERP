(() => {
  ...
  const stages = [
    ...
    ["extracting", "提取申报表", "逐页识别税号"],
    ["normalizing", "整理属性", "继承父级与字段角色"],
    ["validating", "校验税号", "重复与层级"],
    ["writing", "生成证据包", "JSONL 与清单"],
    ["completed", "完成", "等待人工审核"]
  ];
  ...
  function renderNodes(nodes) {
    ...
    target.innerHTML = nodes.slice(-8).reverse().map(node => `<tr>
      <td>${esc(node.code)}</td><td>${esc(node.name)}</td><td>${esc(node.heading_code || "—")}</td><td>${esc(node.page || "—")}</td><td>${esc((node.classification_attributes || []).join("、") || "—")}</td>
    </tr>`).join("");
  }
  ...
  function render(job) {
    ...
    $("downloaded").textContent = formatBytes(job.source_bytes || job.downloaded_bytes);
    $("download-total").textContent = job.sha256 ? `SHA-256 ${String(job.sha256).slice(0, 12)}…` : "本地文件";
    $("pages").textContent = `${Number(job.pages_processed || 0)} / ${job.page_count || 642}`;
    $("nodes").textContent = Number(job.profiles_found || 0).toLocaleString("zh-CN");
    $("leaf-codes").textContent = Number(job.codes_found || 0).toLocaleString("zh-CN");
  ...
  async function poll() {
    ...
    try {
      const response = await fetch(`/api/tariff-declaration/job?job_id=${encodeURIComponent(state.jobId)}`, {cache: "no-store"});
  ...
  async function start() {
    ...
    if (mode !== "demo" && !confirmed) {
      setActionMessage("局部或全量模式必须先勾选明确确认", true);
      return;
    }
    $("start").disabled = true;
    setActionMessage(mode === "demo" ? "正在创建演示任务…" : mode === "slice" ? "正在创建局部任务…" : "正在创建全量任务…");
    try {
      const response = await fetch("/api/tariff-declaration/jobs", {
        method: "POST", headers: {"Content-Type": "application/json"},
        body: JSON.stringify({mode, confirmed, source_url: $("source-url").value, chapters: $("chapters").value}),
      });
      const body = await response.json();
  ...
  async function cancel() {
    ...
    try {
      const response = await fetch("/api/tariff-declaration/jobs/cancel", {
  ...
  $("mode").addEventListener("change", event => {
    const external = event.target.value !== "demo";
    $("confirm-full").disabled = !external;
    $("chapters").disabled = event.target.value !== "slice";
    $("start").textContent = event.target.value === "full" ? "启动全量任务" : event.target.value === "slice" ? "启动局部任务" : "启动演示任务";
    setActionMessage(external ? "勾选确认后读取工作区内的 PDF" : "演示模式不会读取 PDF");
  });
