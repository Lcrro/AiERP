...
- Capability 规划性能第一轮收敛：发现结果不再用零分能力凑数；唯一高置信命中时由 Runtime 自动加载对应 Guide，仍保留歧义场景的模型选择。单场景真实 DeepSeek 抽样中，材料申请由约 `11` 步降至 `7` 步。
...
1. 继续分析材料申请与库存链路中剩余的实体解析和契约读取往返，保持渐进披露与确定性边界不变。
